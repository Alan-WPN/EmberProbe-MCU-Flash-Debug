...
const {
    ...
    requestBuffer, requestFile, extractPackEntry, safeZipName, deviceMatches, vendorMatches, packageScore
} = require("../src/services/officialSvdService");

const VALID_SVD = Buffer.from(`<?xml version="1.0"?><device><name>STM32F40x</name><vendor>STMicroelectronics</vendor><version>1.0</version><peripherals><peripheral><name>GPIOA</name><baseAddress>0x40020000</baseAddress><registers><register><name>MODER</name><addressOffset>0</addressOffset><size>32</size></register></registers></peripheral></peripherals></device>`);
...
(async () => {
    ...
    const compatibleIdentity = new DeviceIdentityService().resolve({ target: "stm32f4x.cfg", chipInfo: { chip: "GD32F407VG", authenticity: "compatible", compatVendor: "GigaDevice" } });
    assert.strictEqual(compatibleIdentity.vendor, "GigaDevice");
    assert.strictEqual(compatibleIdentity.compatible, true);
    assert.strictEqual(new DeviceIdentityService().resolve({ target: "nordic/nrf52.cfg", chipInfo: { deviceId: "0x1234", core: "Cortex-M4" } }).confidence, "fingerprint");
    ...
    assert.throws(() => validateSvdBuffer(Buffer.from("<!DOCTYPE x [<!ENTITY e SYSTEM 'file:///etc/passwd'>]><device/>")), error => error.code === "UNSAFE_XML");
    assert.throws(() => validateSvdBuffer(VALID_SVD, { device: "GD32F407VG", vendor: "GigaDevice" }), error => /MISMATCH/.test(error.code));
    assert.throws(() => validateSvdBuffer(VALID_SVD, { device: "STM32F407VG", vendor: "GigaDevice" }), error => error.code === "SVD_VENDOR_MISMATCH");
    ...
    const context = {
        ...
        globalState: {
            ...
            async update(key, value) { state[key] = value; }
        }
    };
    ...
    const first = await library.importFile(source, { source: "test" }, { device: "STM32F407VG", vendor: "STMicroelectronics" });
    ...
    const folderA = { toString: () => "file:///a" }, folderB = { toString: () => "file:///b" };
    ...
    const hierarchy = parseXml(Buffer.from(`<package><vendor>Keil</vendor><devices><family Dfamily="STM32F4" Dvendor="STMicroelectronics"><debug svd="SVD/STM32F40x.svd" Pname="CM4"/><subFamily DsubFamily="STM32F407"><device Dname="STM32F407VG"/></subFamily></family></devices></package>`), "test PDSC").package;
    ...
    assert.strictEqual(collectLicenses(parseXml(Buffer.from(`<package><licenseSets><licenseSet gating="true"><license title="A" spdx="MIT"/></licenseSet></licenseSets></package>`), "license").package).gating, true);
    assert.strictEqual(collectLicenses({ license: "LICENSE.txt" }).items[0].name, "LICENSE.txt");
    assert.strictEqual(latestRelease(parseXml(Buffer.from(`<package><releases><release version="1.0.0-beta"/><release version="1.0.1"/></releases></package>`), "release").package, "0.0.1").version, "1.0.1");
    ...
    const server = http.createServer((req, res) => {
        if (req.url === "/index.pidx") {
            ...
            res.setHeader("Content-Type", "application/xml"); res.end(body); return;
        }
        ...
        if (req.url === "/Keil.STM32F4xx_DFP.1.2.3.pack") {
            res.setHeader("Content-Length", String(pack.length)); res.end(pack); return;
        }
        if (req.url === "/slow") {
        ...
        if (req.url === "/redirect") { res.statusCode = 302; res.setHeader("Location", "/index.pidx"); res.end(); return; }
        if (req.url === "/too-large") { res.setHeader("Content-Length", "999999"); res.end("x"); return; }
    ...
    try {
        const progress = [];
        const official = new OfficialSvdService({ indexUrl: `${baseUrl}/index.pidx`, allowHttpLocalhost: true, timeoutMs: 2000 });
        const candidates = await official.discover({ device: "STM32F407VG", vendor: "STMicroelectronics", exact: true }, { onProgress: item => progress.push(item) });
        ...
        await assert.rejects(requestBuffer(`${baseUrl}/missing`, { allowHttpLocalhost: true }), error => error.code === "HTTP_ERROR");
        await assert.rejects(requestBuffer(`${baseUrl}/too-large`, { allowHttpLocalhost: true, maxBytes: 10 }), error => error.code === "DOWNLOAD_TOO_LARGE");
        ...
        await assert.rejects(requestFile(`${baseUrl}/missing`, path.join(storage, "missing.bin"), { allowHttpLocalhost: true }), error => error.code === "HTTP_ERROR");
        await assert.rejects(requestFile(`${baseUrl}/too-large`, path.join(storage, "large.bin"), { allowHttpLocalhost: true, maxBytes: 10 }), error => error.code === "DOWNLOAD_TOO_LARGE");

        const packPath = path.join(storage, "test.pack");
        await fs.promises.writeFile(packPath, pack);
        await assert.rejects(extractPackEntry(packPath, "SVD/missing.svd"), error => error.code === "SVD_NOT_IN_PACK");

        const abort = new AbortController();
        const pending = requestBuffer(`${baseUrl}/slow`, { allowHttpLocalhost: true, signal: abort.signal, maxBytes: 1024 * 1024 });
        setTimeout(() => abort.abort(), 30);
        await assert.rejects(pending, error => error.code === "DOWNLOAD_CANCELLED");
        const fileAbort = new AbortController();
        const filePending = requestFile(`${baseUrl}/slow`, path.join(storage, "cancelled.pack"), { allowHttpLocalhost: true, signal: fileAbort.signal, maxBytes: 1024 * 1024 });
        ...
        const discoverPending = new OfficialSvdService({ indexUrl: `${baseUrl}/cancel-index.pidx`, allowHttpLocalhost: true, timeoutMs: 2000 })
            .discover({ device: "STM32F407VG", vendor: "STMicroelectronics", exact: true }, { signal: discoverAbort.signal });
        setTimeout(() => discoverAbort.abort(), 30);
        await assert.rejects(discoverPending, error => error.code === "DOWNLOAD_CANCELLED", "catalog/PDSC cancellation must not be swallowed and retried");
        assert.throws(() => requestBuffer("http://example.com/file", {}), error => error.code === "INSECURE_SOURCE");
    } finally {
