...
assert.match(provider, /servertype:\s*'external'/, "EmberProbe-launched sessions should reuse the managed OpenOCD server");
assert.match(provider, /gdbTarget:\s*managed\.gdbTarget/);
assert.match(provider, /showDevDebugOutput:\s*'none'/, "managed sessions must suppress Cortex-Debug protocol logging");
assert.match(provider, /__emberprobeManagedToken/);
assert.strictEqual(
    (provider.match(/snapshotReady: this\._debugBridge\.hasSession \? this\._debugBridge\.snapshotReady/g) || []).length,
    2,
...
assert.ok(liveWatch.indexOf("configure -work-area-backup 1") < liveWatch.indexOf("'-c', 'init'"));
...
assert.match(extension, /onWillReceiveMessage:[\s\S]*handleDebugAdapterRequest/,
...
assert.match(provider, /event\.transition && event\.transition !== 'continue'/,
