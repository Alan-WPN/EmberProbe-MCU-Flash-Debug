...
const DIAGNOSTICS = {
    ...
    INVALID_WRITE_VALUE: ['write_safety', '写入值超出目标类型范围或非法。', ['按变量类型（u8/i8/u16/i16/u32/i32/f32/u64/i64/f64）提供范围内的值；64 位整数请使用十进制字符串保持精度。'], false],
    LIVE_PANEL_NOT_OPEN: ['live_watch', '没有可用的实时图表面板，无法读取其历史缓冲。', ['打开“实时变量图表”面板并采集数据后重试。'], true],
    INVALID_CSV_RANGE: ['live_watch', 'CSV 导出时间范围无效。', ['优先使用 --last，或使用带日期且以 Z 结尾的 ISO 8601 UTC 时间戳。', '裸 HH:MM[:SS] 按运行 Skill 的本机时区解析；检查 details.requestedRange.resolvedUtc 确认实际区间。'], false],
    CSV_SERIES_NOT_FOUND: ['live_watch', '所选曲线不在目标图表面板的历史缓冲中。', ['检查变量名和 --panel 编号，或省略 --variables 导出全部有数据的曲线。'], false],
    CSV_EXPORT_EMPTY: ['live_watch', '所选曲线或时间区间内没有可导出的采样点；常见原因是把 UTC 时刻作为本机 HH:MM 传入。', ['优先用 --last 指定相对区间，或传入以 Z 结尾的完整 ISO 8601 UTC 时间戳。', '检查 details.requestedRange.resolvedUtc 与 localTimeZone，再判断是否需要扩大范围。'], true],
